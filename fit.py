...
def make_fit(X, Y, Y_e, A_start):
    ...
    par, cov=curve_fit(
                        ...
                        lambda Z,AV,AS,AA: SEMF(Z,A_start,AV,AS,Ac,AA),
                        xdata = X, ydata = Y,
...
def make_plot(X, Y, Y_e, A_start, par, par_e, chi2):
    ...
    textAV = '$A_V$ = ' + format(par[0], '1.1f') + ' $\pm$ ' + format(par_e[0],'1.1f') + ' MeV'
    textAS = '$A_S$ = ' + format(par[1], '1.1f') + ' $\pm$ ' + format(par_e[1],'1.1f') + ' MeV'
    #textAC = 'AC = ' + format(par[2], '1.4f') + ' $\pm$ ' + format(par_e[2],'1.4f') + ' MeV'
    textAC = '$A_C$ = ' + format(Ac, '1.3f') + ' MeV' + ' fixed'
    textAA = '$A_A$ = ' + format(par[2], '1.3f') + ' $\pm$ ' + format(par_e[2],'1.3f') + ' MeV'
    ...
    # plot fit
    ax.plot(np.arange(XMIN, XMAX, 0.01), SEMF(np.arange(XMIN, XMAX, 0.01), A_start, par[0], par[1], Ac, par[2]), 
