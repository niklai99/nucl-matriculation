...
def make_plot(X, Y, Y_e, A_start, par):
...
def main():
    ...
    Z_start = 44 # Z di partenza
    A_start = 101 # A di partenza

    # get data
    X, Y, Y_e = fetch_data(A_start,Z_start)

    par, par_e = make_fit(X, Y, Y_e, A_start)

    fig, ax = make_plot(X, Y, Y_e, A_start, par)
